...
def test_marc21_matcher_books():
    ...
    assert serial.model == matcher(serial_blob1, models_entrypoint)
    assert multipart.model == matcher(multipart_blob1, models_entrypoint)
    assert multipart.model == matcher(multipart_blob2, models_entrypoint)
    assert journal.model == matcher(journal_blob, models_entrypoint)
