...
def importer_test_data(app, db, es_clear):
    ...
    data = load_json_from_datadir("existing_series.json", relpath="importer/json")
    Series = current_app_ils.series_record_cls
    series = _create_records(db, data, Series, SERIES_PID_TYPE)

    data = load_json_from_datadir("existing_documents.json", relpath="importer/json")
    Document = current_app_ils.document_record_cls
    documents = _create_records(db, data, Document, DOCUMENT_PID_TYPE, is_rdm=True)

    data = load_json_from_datadir("existing_eitems.json", relpath="importer/json")
